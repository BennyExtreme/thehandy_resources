// ==UserScript==
// @name         theHandy Support for Web
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @downloadURL  https://raw.githubusercontent.com/BennyExtreme/thehandy_resources/master/web.js
// @updateURL    https://raw.githubusercontent.com/BennyExtreme/thehandy_resources/master/web.js
// @description  Modern web support for theHandy device
// @author       BennyExtreme
// @match        *://*/*
...
(function() {
	...
	const WEBSITE_CONFIGS = [{
		...
		},
		{
			...
			"video_xpath": "//video[@id='gelcomVideoPlayer']"
		},
		{
			"url_regex": "https://www\\.porntrex\\.com/video/.*/.*",
		...
		{
			"url_regex": "https://www\\.erome\\.com/a/.*",
	...
	class HandyController {
		...
		onReady(connectionKey, scriptUrl) {
			...
			GM_xmlhttpRequest({
				method: "GET",
				url: this.urlAPI + "/syncPrepare?" + new URLSearchParams(params).toString(),
				onload: (response) => {
		...
		setOffset(ms) {
			...
			GM_xmlhttpRequest({
				method: "GET",
				url: this.urlAPI + "/syncOffset?" + new URLSearchParams(params).toString(),
				onload: (response) => {
					const result = JSON.parse(response.responseText);
		...
		onPlay(videoTime) {
			...
			GM_xmlhttpRequest({
				method: "GET",
				url: this.urlAPI + "/syncPlay?" + new URLSearchParams(params).toString(),
				onload: (response) => {
					const result = JSON.parse(response.responseText);
	...
	class HandyWebApp {
		constructor() {
			this.handy = new HandyController();
		...
		async initialize() {
			...
			const matchedConfig = this.findMatchingWebsiteConfig();
			if (!matchedConfig) {
				return; // Not a supported website
			}

			// Wait for video element to load
			this.videoElement = await this.waitForVideoElement(matchedConfig.video_xpath);
			if (!this.videoElement) {
				console.log('theHandy: Video element not found');
				return;
			}
			...
			console.log('theHandy: Initialized successfully');
		}

		findMatchingWebsiteConfig() {
		...
		async waitForVideoElement(xpath, maxAttempts = 50) {
